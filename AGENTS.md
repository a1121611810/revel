...
`.github/workflows/ci.yml` 在 push/PR 到 main 时触发，包含 4 个并行 job：

1. **lint** — `oxlint src/`
2. **format** — `oxfmt src/ --check`
3. **test** — `vitest run`（单元测试）
4. **build** — `pnpm build:mac`（electron-forge make）

E2E 测试（`pnpm test:e2e`）仅本地运行，不纳入 CI（避免 Linux/macOS 兼容性和 Mole CLI 安装依赖）。

### 自动发布（release-please.yml + release.yml）

每次合并到 main 分支时自动发布新版本：

```
push to main（提交必须含 feat: 或 fix: 前缀）
  → release-please.yml 触发
    → release-please-action 创建 release PR（含版本号 bump + changelog）
    → auto-merge 自动合并 release PR
    → release-please-action 再次运行，创建 Git tag + GitHub Release
      → release.yml 触发（on: tags v*）
        → 构建 macOS x64 + arm64 的 dmg/zip
        → 上传到 GitHub Release
```

### 提交规范（Conventional Commits）

**所有合并到 main 的提交必须遵循 Conventional Commits 格式**，否则 release-please 不会创建 release PR，发布流程不会触发。

- `feat: 添加新功能` → 触发 **minor** 版本升级（1.0.0 → 1.1.0）
- `fix: 修复某个 bug` → 触发 **patch** 版本升级（1.0.0 → 1.0.1）
- `feat!: 破坏性变更` 或 `fix!: 破坏性变更` → 触发 **major** 版本升级（1.0.0 → 2.0.0）

其他前缀（`chore:`、`docs:`、`refactor:`、`test:`、`ci:`、`build:`）不会触发版本升级。

合并 PR 时使用 **Squash and merge**，PR 标题作为最终 commit message。例如：
- PR 标题：`feat: 添加暗色主题支持` → 触发 minor 升级
- PR 标题：`fix: 修复托盘图标不显示的问题` → 触发 patch 升级
