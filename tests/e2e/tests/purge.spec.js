...
test.describe('PurgeView E2E', () => {
  ...
  test('should navigate to purge view', async () => {
    ...
    await window.locator('text=项目清理').click()
    await window.waitForTimeout(500)

    await expect(window.getByRole('heading', { name: '项目清理' })).toBeVisible()
  })
})
